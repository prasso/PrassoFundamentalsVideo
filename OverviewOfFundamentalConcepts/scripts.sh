...
- A Prasso site is both a business information site and a Prasso API site. The API serves the Prasso apps." --model_name "tts_models/en/ljspeech/glow-tts" --vocoder_name "vocoder_models/en/ljspeech/univnet" --out_path 08.wav
tts --text " Sites have site pages. These can be created and maintained using a built-in visual editor based on the GrapesJS opensource project.
- Sites are the landing page of the App home website. " --model_name "tts_models/en/ljspeech/glow-tts" --vocoder_name "vocoder_models/en/ljspeech/univnet" --out_path 081.wav
...
- An app is configured with tabs that point to views." --model_name "tts_models/en/ljspeech/glow-tts" --vocoder_name "vocoder_models/en/ljspeech/univnet" --out_path 09.wav
tts --text "An app is identified at the backend by the host of the request.
- The host is associated with the site, the site is associated with the app." --model_name "tts_models/en/ljspeech/glow-tts" --vocoder_name "vocoder_models/en/ljspeech/univnet" --out_path 091.wav
...
tts --text "This concludes the introduction to Prasso Framework. To dig in and find out more, see our other videos such as Prasso for Developers, Prasso Business Use Case, and Prasso Admin Tools" --model_name "tts_models/en/ljspeech/glow-tts" --vocoder_name "vocoder_models/en/ljspeech/univnet" --out_path 11.wav